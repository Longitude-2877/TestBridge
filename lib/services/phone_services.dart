...
class PhoneServices {
  ...
  static const volumeChannel = MethodChannel('elders/volume');

  static Future<bool> setFlashlight(bool on) async {
    try {
      return await _channel.invokeMethod<bool>('setFlashlight', on) ?? false;
    } catch (_) {
      return false;
    }
  }
  ...
  static Future<void> setVolume(double v) async {
    try {
      await _channel.invokeMethod<void>('setVolume', v.clamp(0.0, 1.0));
    } catch (_) {}
  }

  static Future<void> showOverlay(bool show) async {
    try {
      await _channel.invokeMethod<void>('showOverlay', show);
    } catch (_) {}
  }
